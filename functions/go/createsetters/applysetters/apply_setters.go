...
func (as *ApplySetters) visitScalar(object *yaml.RNode, path string) error {
	...
	// str := strings.Split(path, pathDelimiter)

	flag := false

	comment := object.YNode().Value
	...
	if flag {
		object.YNode().LineComment = fmt.Sprintf("# kpt-set: %s", comment)
	}
...
func shouldCreate(pattern string, setters []Setter) bool {
	for _, s := range setters {
		if strings.Contains(pattern, s.Name) {
			return true
		}
	}
	return false
