...
func (p *NamespaceTransformer) SetDependsOnMap(objects []*fn.KubeObject) {
	...
	for _, o := range objects {
		...
		key := fmt.Sprintf(dependsOnKeyPattern, group, o.GetKind(), o.GetName())
...
func (p *NamespaceTransformer) UpdateAnnotation(o *fn.KubeObject){
	...
	dependsOnkey := fmt.Sprintf(dependsOnKeyPattern, segments[groupIdx], segments[kindIdx], segments[nameIdx])
