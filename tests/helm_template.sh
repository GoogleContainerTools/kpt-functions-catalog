...
#TODO: avoiding Helm command template my-haproxy-ingress /source/incubator/haproxy-ingress results in error: WARNING: This chart is deprecated
(cd helm-charts; git checkout 367292a)
kpt fn source example-configs |
...
kpt fn source example-configs |
  kpt fn run --network --image gcr.io/kpt-functions/helm-template:"${TAG}" -- chart=incubator/kafka chart-repo=incubator chart-repo-url=https://kubernetes-charts-incubator.storage.googleapis.com name=my-kafka | 
...
assert_dir_exists default
ls -la default
#assert_contains_string default/configmap_my-kafka-config.yaml "name: my-kafka-config"
assert_contains_string default/secret_my-redis.yaml "name: my-redis"
